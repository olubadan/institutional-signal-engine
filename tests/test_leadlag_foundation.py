...
def test_temporal_evidence_contract_preserves_existing_event_clocks():
    ...
    event = CanonicalEvent(
        ...
        received_timestamp=source + timedelta(milliseconds=40),
        normalized_timestamp=source + timedelta(milliseconds=41),
        sequence=7,
        payload={"contract": {"root": "AAPL"}},
    )
    evidence = TemporalEvidence.from_canonical_event(
        event,
        received_monotonic_ns=123,
        canonical_acceptance_timestamp=source + timedelta(milliseconds=42),
...
def test_temporal_evidence_reads_runtime_capture_fields_without_collapsing_clocks():
    ...
    with pytest.raises(ValueError):
        clock_domain_duration_seconds(
            ...
            start_domain="provider_event_clock",
            end_domain="local_wall_clock",
        )


def test_local_provider_normalization_captures_monotonic_receive_without_network():
    provider = AlpacaEquitiesProvider(
        "wss://offline.invalid/stream", "fixture-key", "fixture-secret"
    )
    event = provider._normalize(
        ...
        ingress_wall_timestamp=datetime(2026, 1, 2, 14, 30, tzinfo=UTC),
        ingress_monotonic_ns=123,
    )
...
def test_t0_valid_order_and_actionable_clock_are_distinct_from_onset():
    base = datetime(2026, 1, 2, 14, 30, tzinfo=UTC)
    lifecycle = T0Lifecycle(
        base, base + timedelta(seconds=1), base + timedelta(seconds=2), base + timedelta(seconds=3)
    )
...
def test_t0_rejects_lookahead_clock_ordering():
    ...
    invalid_emit = T0Lifecycle(
        base, base + timedelta(seconds=1), base + timedelta(seconds=2), base + timedelta(seconds=1)
    )
    assert "emit_before_qualify" in invalid_emit.validation_errors()
    invalid_qualify = T0Lifecycle(
        base, base + timedelta(seconds=1), base - timedelta(seconds=1), None
    )
...
def test_lab_variations_are_seeded_and_ground_truth_is_not_in_visible_events():
    scenario = generate_synthetic_scenario(
        99,
        LeadLagOutcome.OPTIONS_FIRST.value,
        30.0,
        jitter_ms=20,
        ...
        changing_volatility=True,
    )
    assert "true_lag_seconds" not in {
        key for event in scenario.visible_events for key in event.payload
    }
...
def test_out_of_order_arrival_is_real_and_reconstructible_separately_from_provider_time():
    ...
    provider_order = [
        event.event_id
        for event in sorted(
            scenario.visible_events, key=lambda event: (event.provider_timestamp, event.sequence)
        )
    ]
