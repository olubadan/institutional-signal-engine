...
| REQ-AUD-001 — Durable decision and execution lineage | SRS FR-AUD-001/002 | `persistence.py`, `pipeline.py`, `replay.py` | AT-700 | 3, 7 | Verified for corrected run `eba46d34-4335-486d-9929-33863f44b331`: 15,381 accepted events, 15,131 decisions, and field-by-field replay equality; run IDs, ingest order, decision order, and mapping provenance persist. This does not claim all historical runs are replayable. |
| REQ-OBS-001 — Operational and performance telemetry | SRS FR-OBS-001 | `src/.../observability.py`, `pipeline.py`, `live_smoke.py` | AT-701 | 3, 7 | Verified for secret-free loopback health/status and per-provider/event-kind timing distributions preserving source, receive, processing, age-at-receipt, and conversion flags |
