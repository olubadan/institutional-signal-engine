...
| REQ-SIG-001 — Authoritative indicators and S/F/R/E | Owner formal model | `03_Formal_Mathematical_Model.md`, `src/.../signals.py`, `indicators.py` | AT-101, AT-500 | 3, 5 | Partially verified: specified gates and requested equity arithmetic are implemented; the formal model does not define eligible trades, ask-side classification, or five-day resistance/high |
...
| REQ-AUD-001 — Durable decision and execution lineage | SRS FR-AUD-001/002 | `src/.../persistence.py`, `pipeline.py`, `replay.py` | AT-700 | 3, 7 | Partially verified: live pipeline calls event/decision repositories and PostgreSQL event reads are typed; live VM round-trip and field-by-field replay evidence remain pending |
