...
| REQ-DATA-001 — Resilient Alpaca equity stream | SRS FR-DATA-001/004 | `src/.../providers/alpaca.py` | AT-400, AT-401, SESSION-0009 | 3–4 | Verified: contract/feed-name/handshake tests pass; live authentication succeeded and 7,500 events arrived in the bounded smoke |
| REQ-DATA-002 — Resilient replaceable options stream | SRS FR-DATA-002/004 | `src/.../providers/thetadata.py`, `infra/vast/*thetadata*` | AT-400, AT-401, SESSION-0009 | 3–4 | Blocked: Terminal authenticated, active, loopback-only, and acknowledged the request, but the account is Options Standard while the requested bulk stream requires Options Pro; contract discovery returned HTTP 500 |
