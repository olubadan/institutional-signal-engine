...
| REQ-DATA-001 — Resilient Alpaca equity stream | SRS FR-DATA-001/004 | `src/.../providers/alpaca.py`, `indicators.py`, `pipeline.py`, `historical.py` | AT-400, AT-401, SESSION-0010/0011 | 3–4 | Verified for split-adjusted completed-session bootstrap, previous-close/VWAP/RVOL, same-minute benchmark alignment, resistance provenance, live historical bootstrap, and signal-only smoke |
...
| REQ-SIG-001 — Authoritative indicators and S/F/R/E | Owner formal model plus owner SESSION-0010 definitions | `03_Formal_Mathematical_Model.md`, `indicators.py`, `thetadata_conditions.py`, `signals.py`, `sweeps.py` | AT-101, AT-500, SESSION-0010/0011 | 3, 5 | Verified for Decimal indicators, split-adjusted bootstrap, hierarchical resistance, relative-strength gate wiring, fail-closed baselines, owner-authorized fixed-window sweep clustering, session sweep thresholds, correction handling, freshness expiry, and hermetic replay tests; live smoke did not naturally produce a qualifying sweep |
