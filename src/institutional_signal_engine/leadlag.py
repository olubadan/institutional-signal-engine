...
def _price_at(
    timestamp: datetime, onset: datetime | None, rng: random.Random, changing: bool
) -> float:
    baseline = 100.0
...
def _fixture_response_time(events: tuple[SyntheticEvent, ...]) -> datetime | None:
    equity = sorted(
        (event for event in events if event.stream == "equity"), key=lambda e: e.provider_timestamp
    )
    if len(equity) < 2:
        return None
...
def evaluate_synthetic_scenario(scenario: SyntheticScenario) -> TemporalEvaluation:
    ...
    return TemporalEvaluation(
        ...
        out_of_order_count=sum(
            1 for left, right in pairwise(events) if right.sequence < left.sequence
        ),
